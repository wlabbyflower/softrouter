...
[点击下载仓库内置的 LZCWrt v0.1.8](../../../packages/lzcwrt/cloud.lazycat.app.lzcwrt-v0.1.8.lpk)
...
- 上传本地文件：通过自定义镜像文件运行旁路由实例。`v0.1.8` 起可以从当前设备选择文件，也可以从懒猫网盘选择文件，选择后会走同一上传流程。
...
- 新建实例会默认关闭 OpenWrt 兼容系统侧的 DHCP，保留 DNS 服务，降低和主路由 DHCP 冲突的风险。
- 如果确实需要让旁路由接管 DHCP，请进入对应系统的网络或 DHCP 页面手动开启，并先确认主路由配置不会冲突。

`v0.1.8` 起，LZCWrt 提供 LuCI 代理入口。优先使用实例卡片里的**进入 LuCI**按钮；如果遇到某些页面跳转到写死地址或访问异常，再复制网络配置中的 IP 地址直接访问。
